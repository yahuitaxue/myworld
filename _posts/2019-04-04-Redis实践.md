...
redis中，使用pipeline，事务，lua脚本实现多命令的原子性的区别
	1. Pipeline：Pipeline 是一种将多个 Redis 命令一次性发送给服务器执行的机制。通过将多个命令打包成一个批处理，然后一次性发送给 Redis，可以减少网络开销和请求-响应延迟，提高性能。但是，Pipeline 并不能保证原子性，因为 Redis 会按照命令的顺序执行命令，如果发生错误并不会中途终止执行。
	2. 事务：Redis 的事务使用 MULTI、EXEC 和 DISCARD 命令组合起来实现。通过 MULTI 命令开始一个事务，在 EXEC 命令被调用之前，所有的命令都只是被放入队列中，而不会真正执行。只有在 EXEC 被调用时，Redis 才会依次执行事务中的命令。在执行期间，如果发生了错误，Redis 会终止执行，并返回错误信息。事务的原子性体现在在 EXEC 命令执行期间没有其他命令被插入。
	3. Lua 脚本：Redis 支持使用 Lua 脚本执行一段完整的 Lua 代码，从而可以实现复杂的逻辑和多个命令的原子操作。在执行 Lua 脚本期间，Redis 会以单线程的方式执行，保证了原子性。另外，Lua 脚本还可以被 Redis 预编译和缓存，提高执行效率。
分布式锁
	乐观锁：
		使用WATCH来构建基本乐观锁
