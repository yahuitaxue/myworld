...
2.高级
	1.Go编程进阶(必会)
				...
				整体流程：
					1.始化
						...
						4.根据B去创建去创建桶 (bmap对象)并存放在buckets数组中，当前bmap的数量应为2.
							...
							当B>=4时，根据B创建桶的个数的规则为:2+2B-4(标准桶+溢出桶) I
		...
		2.GMP模型(必会)(GMP模型就是goroutine的协程调度器,主要是工作在用户空间,但是底层的M是会进行切换的,也就是在用户空间与内核空间进行切换,主要还是看运行的是用户程序还是系统程序)
			<span class="image featured"><img src="{{ 'assets/images/other/go_GMP.jpg' | relative_url }}" alt="" /></span>
			备注:
				Go语言是原生支持语言级并发的，这个并发的最小逻辑单元就是goroutine。goroutine就是Go语言提供的一种用户态线程，当然这种用户态线程是跑在内核级线程之上的。当我们创建了很多的goroutine，并且它们都是跑在同一个内核线程之上的时候，就需要一个调度器来维护这些goroutine，确保所有的goroutine都使用cpu，并且是尽可能公平的使用cpu资源。这个调度器的原理以及实现值得我们去深入研究一下。支撑整个调度器的主要有4个重要结构，分别是M、G、P、Sched，前三个定义在runtime.h中，Sched定义在proc.c中。
				G：
			...
			内存中,内核空间频繁切换,导致CPU频繁切换线程,也导致性能的浪费,所以就出现以下几种情况
				(将内存空间分为用户空间与内核空间,CPU只针对内核空间可见)
				1.为了避免CPU频繁切换,就出现一个线程(thread)通过协程调度器连接多个协程(co-routine),形成绑定关系
					问题:如果一个协程长时间占用,则导致其他协程不能被执行的问题
				2.一个线程(thread)通过协程调度器连接一个协程(co-routine),形成绑定关系
					问题:虽然不会出现1的问题,但是还是会有CPU频繁切换的问题
				3.多个线程(thread)通过协程调度器连接多个协程(co-routine),形成绑定关系
					由于内核空间用户无法控制,所以此时协程调度器的性能就直接影响整个系统的调度效率,此时就产生了golang的协程开发
			早期golang协程调度器
				只有全局对垒
			...
			其中:
				...
				在整个过程中，G0和M0的执行流程如下：
					...
					需要注意的是，G0和M0并不是固定的编号，它们只是代表Goroutine和Machine的概念。在实际运行中，可能会有多个Goroutine和Machine，并且它们的编号会动态变化。
			7.开始执行程序
				1.创建进程
				2.创建线程M0
		...
		6.并发编程(必会)
			...
			5.context
				...
				3.context.WithTimeout(parent, time.Millisecond*100) 如果父子都设置截止时间,则会根据时间较短的那个结束执行 / 手动调用context.WithCancel(context.TODO())来手动结束
					...
					ctx1, can := context.WithTimeout(ctx, time.Second * 5)
					go cancel(ctx1) // 正常的for + select 来检测<-ctx.Done()
					time.Sleep(time.Second * 2)
					can() // 这里是2秒后手动发送取消信号(ctx.Err()的值是context canceled),而不是等待5秒自动取消(ctx.Err()的值是context deadline exceeded)
				<span class="image featured"><img src="{{ 'assets/images/other/go_context.jpg' | relative_url }}" alt="" /></span>
			6.sync
			7.原子操作 atomic (主要解决数据竟态问题)
				(数据竟态)
					var counter int32
					func main() {
						const grs = 2
						...
						for g := 0; g < grs; g++ {
							go func() {
								for i := 0; i < 2; i++ {
									...
									fmt.Println("logging") // 此处增加了日志打印，有IO操作，此时会发生调度
									counter = value        //进行了一次设置，此时其他的cpu的`cache-line` 被设置为dirty
									// atomic.AddInt32(&counter, 1) 解决问题,需要用到atomic原子操作(当然,使用sync加锁也是可以的)
								}
								wg.Done()
							}()
						}
					...
					(其中fmt.Println("logging")有无这一行结果却不一样)
			...
			tips:
				1.无缓冲的chan在单个线程中读/写是会出现阻塞,但是使用协程,就只会阻塞当前的协程,不会影响主线程
				2.interface中是有两个字段,一个是tab所属类型,一个是data指向的值,所以判断是否为nil的时候,不仅判断值,还要判断类型
		7.测试(必会)
			1.单元测试
			2.压力测试
			3.覆盖测试
			...
			5.go程序调试
				1.使用dlv工具
					1.top查看阻塞进程
					2.~go/bin/dlv attach 进程号
					3.grs查看当前所有协程
					4.gr 6 切换到协程6上(前面有一个*号)
					5.bt查看栈回溯,也就是执行的命令
		8.数据结构与算法(必会)
		9.操作系统(必会)
			1.熟悉5种网络IO模型原理
		...
		1.限流
			1.令牌桶算法(Token Bucket Algorithm)：使用一个固定容量的桶来存放令牌，每当有请求到达时，消耗一个令牌，只有当桶中有足够的令牌时才能继续服务，否则请求被拒绝。
				type TokenBucket struct {
					capacity int64 // 令牌桶的最大容量
					fillInterval time.Duration // 每个令牌的填充间隔
					availableTokens int64 // 当前可用的令牌数量
			...
			7.go限流机制
				1.golang.org/x/time/rate(包)
					limiter := rate.NewLimiter(rate.Every(time.Millisecond*100), 1)
					for {
						limiter.WaitN(context.Background(), 1)     // 1.如果没有申请到令牌,则会阻塞
						...                                        // 1.处理逻辑
						reserve := limiter.ReserveN(time.Now(), 1) // 2.当前时间有没有令牌
						time.Sleep(reserve.Delay())                // 2.获取当前没有令牌,还需要等待多久才会有
						...                                        // 2.处理逻辑
						if limiter.AllowN(time.Now(), 1) {         // 3.当前时间有没有令牌,如果没有则立即返回
							...                                    // 3.处理逻辑
						}
					}
				2.自定义限流
					...
					gin中使用
						...
						1.首先每次请求都会进行加锁,解锁,防止并发问题
						2.每次根据当前请求时间差进行计算,需要补充多少的token
						3.计算桶中token与桶大小size进行比较
						4.请求成功桶中token数量减少1
