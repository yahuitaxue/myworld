...
go中的锁
	...
	rwm.RUnlock()
docker底层实现原理
	1. Linux命名空间（Namespace）：Docker使用了几个不同的命名空间，包括进程（PID）、网络（Net）、文件系统（Mount）、主机名（UTS）、IPC、用户（User）等，使得容器中的进程拥有自己独立的资源视图，避免与宿主机或其他容器之间相互干扰。
	2. 控制组（cgroups）：Docker利用了控制组来限制容器的资源使用，如CPU、内存、磁盘等。通过将容器中的进程组织到特定的控制组中，并为每个控制组分配相应的资源配额，以实现对容器资源的限制和隔离。
	3. 联合文件系统（UnionFS）：Docker使用了联合文件系统，将主机的文件系统与容器的文件系统进行分离。联合文件系统通过在宿主机上创建一个或多个可写层，并将其与只读的基础镜像层进行叠加，使得容器可以在只读的基础上进行读写操作，同时也实现了镜像的共享和复用。
	4. 安全加固技术：Docker还使用了一些安全加固技术来增强容器的隔离性。例如，通过seccomp对系统调用进行过滤，禁止或限制容器中的进程执行某些危险的系统调用，以减少容器对主机的潜在威胁。另外，Docker还使用了一些特定的内核参数或功能，如命名空间和cgroups的配置选项，以优化和增强容器的隔离性能。
	总的来说
		Docker通过利用Linux内核的特性和一些安全加固技术，为容器提供了一种轻量级、高效和灵活的隔离环境，使得容器可以在不同的环境中独立运行，并且与宿主机和其他容器之间保持隔离和安全。
集群分布式
...
go里面interface是什么概念
	...
	eface
		表示empty interface，不包含任何方法的空接口
	iface 
		表示 non-empty interface，即包含方法的非空接口
...
MySQL自增ID用完了会出现什么情况
	...
	2.数据不会到20多亿的级别
		...
		2.不采用自增主键
		3.数据迁移
MySQL高度为2的b+tree能存多少数据
	比如主键是bigint,8B
	指针占用6B
	比如一行数据大概1KB
	那么非叶子节点大概有 16384/(8+6) = 1170个指向节点
	那么高度为2的就有 1170 * (16384 / 1024) = 18720条数据
	高度为3的就有 1170 * 1170 * (16384 / 1024)
</pre>