...
隔离原理
    1. 命名空间（Namespace）：使用Linux的命名空间特性，Docker将各个进程的资源（如文件系统、网络、用户ID等）隔离开来。每个容器都有自己独立的命名空间，不同容器间互不可见(比如用户隔离,在容器中是root权限的用户,但是在宿主机上只有有限权限的普通用户)。
    2. 控制组（Control Groups）：Docker使用控制组功能来限制和隔离容器的资源使用，如CPU、内存、磁盘I/O等。这样可以确保一个容器不会耗尽系统的资源，影响其他容器的运行。
    3. 文件系统（File System）：每个容器都有自己的文件系统，类似于一个独立的根目录。Docker使用联合文件系统（UnionFS）来创建容器的文件系统，可以将多个文件系统层叠在一起，形成一个统一的视图。
    4. 网络隔离（Network Isolation）：Docker为每个容器创建一个虚拟网络接口，每个容器都有自己独立的IP地址和端口空间。通过网络隔离，容器间可以相互通信，但不与宿主机或其他容器的网络接口冲突。
